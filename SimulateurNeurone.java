...
class Neurone{
	...
	public String toString(){
		...
		if (getNbConnexions() > 0){
			str+=" en connexion avec\n";
			for(Neurone n:connexions){
				str+="  - un neurone en position "+n.getPosition()+"\n";
			}
		}else{
			str+=" sans connexion\n";
		}
...
class NeuroneCumulatif extends Neurone{
	...
	public void recoitStimulus(double stimulus){
		double signalInterne= getSignal()/getAttenuation()+stimulus;
		super.recoitStimulus(signalInterne);
	}
}
...
class Cerveau{
	...
	public String toString(){
	    String result="\n\n*----------*\n\n";
		result += "Le cerveau contient " + cerveau.size() + " neurone(s)\n";
